app.controller('paidController', ['$scope', '$http','LoggedUser', '$localStorage', '$window','api_url',function($scope, $http, LoggedUser, $localStorage, $window, api_url){
    ...
    $scope.getPaid = function(id){
	    $http({
	      	method: 'GET',
	      	url: api_url.url+'/paid/items/'+id,
	      	headers: {'Authorization': 'Bearer '+$localStorage.key}
	    }).then(
	    	function successCallback(success) {
	    		$scope.detailpaid = success.data;
	    	},
	    	function errorCallback(error) {
	    		$scope.detailpaid = error.data;
	    	}
	    );
    }
